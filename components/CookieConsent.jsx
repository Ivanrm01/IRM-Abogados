...
export default function CookieConsent() {
  ...
  // Texto literal que el visitante tiene delante al decidir. Se guarda en el
  // registro de consentimientos como prueba de que la decisión fue informada.
  const textoMostrado = useCallback(() => {
    const raiz = panelRef.current || bannerRef.current
    if (!raiz) return ''
    const copia = raiz.cloneNode(true)
    copia.querySelectorAll('button, [aria-hidden="true"]').forEach((n) => n.remove())
    // Separa títulos, párrafos y bloques para que el texto no salga pegado
    copia.querySelectorAll('h1, h2, h3, h4, p, div, li, label').forEach((n) => n.append(' '))
    return copia.textContent.replace(/\s+/g, ' ').trim().slice(0, 4000)
  }, [])

  const save = useCallback((value, via) => {
    writeConsent(value, { via, texto: textoMostrado() })
      ...
      >
        <div className={styles.inner}>
          ...
          <div className={styles.actions}>
            <button type="button" className={styles.btnPrimary} onClick={() => save(ALLOW_ALL, 'banner-aceptar')}>
              Aceptar todas
            </button>
            <button type="button" className={styles.btnPrimary} onClick={() => save(DENY_ALL, 'banner-rechazar')}>
              Rechazar todas
            </button>
    ...
    <div className={styles.overlay}>
      ...
      >
        ...
        <div className={styles.panelActions}>
          <button type="button" className={styles.btnPrimary} onClick={() => save(ALLOW_ALL, 'panel-aceptar')}>
            Aceptar todas
          </button>
          <button type="button" className={styles.btnPrimary} onClick={() => save(DENY_ALL, 'panel-rechazar')}>
            Rechazar todas
          </button>
          <button type="button" className={styles.btnGhost} onClick={() => save(prefs, 'panel-guardar')}>
            Guardar selección
          </button>
        </div>
