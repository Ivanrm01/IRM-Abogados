...
export default function ContactoPage() {
  ...
  const faqs = [
    ...
    { q: '¿Atendéis a clientes de toda España?', a: 'Sí . Aunque tenemos sedes físicas en Madrid y Castellón, atendemos a clientes de toda España. La distancia no es ningún obstáculo para recibir un asesoramiento de calidad.' },
      ...
      <section className={styles.hero}>
        ...
        <div className={styles.heroInner}>
          <div>
            ...
            <p className={styles.heroDesc}>Analizamos tu situación y te decimos exactamente qué podemos hacer por ti y cuánto cuesta, antes de que tomes ninguna decisión.</p>
            <div className={styles.promises}>
              {['Respuesta garantizada en menos de 24 horas','Presupuesto cerrado antes de empezar','Hablas directamente con el abogado, no con una centralita','Atendemos clientes de toda España'].map(p => (
                <div key={p} className={styles.promise}><div className={styles.dot}></div>{p}</div>
              ))}
            </div>
          </div>
          <div className={styles.quickLinks}>
            <a href="tel:+34614149465" onClick={() => gaTrack('click_telefono', { ubicacion: 'contacto_hero' })} className={styles.quickCard}>
            ...
            <a href="https://wa.me/34614149465" target="_blank" rel="noopener noreferrer" onClick={() => gaTrack('click_whatsapp', { ubicacion: 'contacto_hero' })} className={styles.quickCard}>
        ...
        <div className={styles.formSide}>
          {sent ? (
            <div className={styles.successBox}>
              ...
              <p>Te responderemos en menos de 24 horas. Mientras tanto, puedes llamarnos al <a href="tel:+34614149465" onClick={() => gaTrack('click_telefono', { ubicacion: 'contacto_confirmacion' })}>+34 614 149 465</a>.</p>
            </div>
          ) : (
            <>
              ...
              <label className={styles.checkLabel}>
                ...
                <span>He leído y acepto la <Link href="/politica-de-privacidad" style={{color:'var(--gold)'}}>política de privacidad</Link>. Mis datos serán tratados con total confidencialidad.</span>
              </label>
        ...
        <div className={styles.infoSide}>
          ...
          {[{icon:'📞',label:'Teléfono',value:'+34 614 149 465',sub:'Lunes a viernes · 8:00 – 19:00',href:'tel:+34614149465'},{icon:'💬',label:'WhatsApp',value:'Escríbenos por WhatsApp',sub:'Respuesta rápida en horario de oficina',href:'https://wa.me/34614149465'},{icon:'✉️',label:'Email',value:'correo@irmabogados.es',sub:'Respondemos en menos de 24 horas',href:'mailto:correo@irmabogados.es'}].map(c => (
            <div key={c.label} className={styles.contactRow}>
              ...
              <div><div className={styles.crLabel}>{c.label}</div><a href={c.href} onClick={() => gaTrack(c.href?.startsWith('tel:') ? 'click_telefono' : c.href?.includes('wa.me') ? 'click_whatsapp' : 'click_email', { ubicacion: 'contacto_lateral' })} className={styles.crValue}>{c.value}</a><div className={styles.crSub}>{c.sub}</div></div>
            </div>
          ))}
          <div className={styles.sedesTitle}>Nuestras sedes</div>
          <div className={styles.sedesGrid}>
            <a href="https://www.google.com/maps/place/?q=place_id:ChIJKUiFqZgpQg0RV-I-ac1bG4s" target="_blank" rel="noopener noreferrer" className={styles.sedeCard}><div className={styles.sedeCity}>Madrid</div><div className={styles.sedeAddr}>C/ José Ortega y Gasset, 84 — 2º C · 28006 Madrid</div><span className={styles.sedeMap}>Ver en mapa →</span></a>
            <a href="https://www.google.com/maps/place/?q=place_id:ChIJtwgvmzv_Xw0RswyxvCi2JAs" target="_blank" rel="noopener noreferrer" className={styles.sedeCard}><div className={styles.sedeCity}>Castellón</div><div className={styles.sedeAddr}>C/ En Medio, 22 — 6º · 12001 Castellón de la Plana</div><span className={styles.sedeMap}>Ver en mapa →</span></a>
          </div>
        </div>
      </div>
