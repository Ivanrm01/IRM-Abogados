...
const api = async (url, opts = {}) => {
  ...
  // 401 con sesion:false = sesión caducada. Otro 401 (contraseña actual mal escrita) es un error normal.
...
export default function Usuarios({ usuario }) {
  ...
  const quitar2FA = async (u) => {
    if (!confirm(`¿Quitar la verificación en dos pasos de ${u.email}?\n\nHazlo solo si ha perdido el móvil y los códigos de recuperación. Podrá entrar solo con su contraseña hasta que la vuelva a activar en Seguridad.`)) return
    try {
      const d = await api('/api/auth/usuarios', { method: 'PATCH', body: JSON.stringify({ id: u.id, quitar2fa: true }) })
      setLista(l => l.map(x => x.id === d.usuario.id ? d.usuario : x))
      flash('Verificación en dos pasos quitada. Pídele que la vuelva a activar en cuanto entre.')
    } catch (e) { setError(e.message) }
  }
          ...
          <>
            <div className={s.tableWrap} style={{ marginBottom: 16 }}>
              <table className={s.table}>
                <thead><tr><th>Nombre</th><th>Email</th><th>Estado</th><th>2 pasos</th><th>Último acceso</th><th className={s.right}>Acciones</th></tr></thead>
                <tbody>
                  {lista.map(u => (
                    <tr key={u.id}>
                      ...
                      <td><span className={`${s.tag} ${u.dos_pasos ? s.tagGreen : s.tagGrey}`} title={u.dos_pasos && u.dos_pasos_desde ? 'Desde el ' + fecha(u.dos_pasos_desde) : ''}>{u.dos_pasos ? 'Activada' : 'No'}</span></td>
                      ...
                      <td className={s.right}>
                        <div style={{ display: 'flex', gap: 6, justifyContent: 'flex-end', flexWrap: 'wrap' }}>
                          ...
                          {u.id !== yo && u.dos_pasos && <button className={`${s.btn} ${s.btnMini}`} onClick={() => quitar2FA(u)}>Quitar 2 pasos</button>}
              ...
              <div className={s.card}>
                ...
                <div style={{ padding: '14px 18px', fontSize: 13, lineHeight: 1.6, color: '#4A5568', fontWeight: 300 }}>
                  <p style={{ margin: '0 0 10px' }}>
                    Cada persona entra con su email y su contraseña y, si la tiene activada, con el
                    código de su móvil (verificación en dos pasos, en <strong style={{ fontWeight: 500 }}>Seguridad</strong>).
                    La sesión se cierra tras 30 minutos sin actividad y, como máximo, a las 12 horas.
                  </p>
                  <p style={{ margin: '0 0 10px' }}>
                    Las contraseñas se guardan cifradas: ni tú ni yo podemos verlas. Si alguien la
                    olvida, le pones una nueva desde aquí y que la cambie al entrar. Al cambiarla se
                    cierran sus sesiones abiertas.
                  </p>
                  <p style={{ margin: '0 0 10px' }}>
                    Si alguien pierde el móvil y sus códigos de recuperación, pulsa «Quitar 2 pasos» en
                    su cuenta y que la vuelva a activar en cuanto entre.
                  </p>
                  <p style={{ margin: 0 }}>
