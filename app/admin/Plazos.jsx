...
const TIPOS = [
  ...
  { id:'otros', via:'otros', label:'Otros (fecha de vencimiento libre)', meses:0, dias:0, ref:'Fecha fijada manualmente', libre:true },
]
...
const VIAS = {
  ...
  otros: { label:'Otros', color:'#6b7280', agostoInhabil:false },
}
...
const calcularVencimiento = (tipo, fechaNotifStr, fechaLibre) => {
  // Tipo "Otros": la fecha de vencimiento es la que el usuario fija manualmente
  if (tipo.libre) return fechaLibre || null
...
export default function Plazos({ prefill }) {
  const isMobile = useIsMobile()
  const [asuntos, setAsuntos] = useState([])
  const [form, setForm] = useState({ asunto:'', cliente:'', tipo:'reposicion', fechaNotif:'', fechaLibre:'' })
  ...
  // Normaliza un registro de Supabase al formato interno del componente
  const normaliza = (r) => ({
    id: r.id,
    asunto: r.asunto,
    cliente: r.cliente || '',
    ...
    via: r.via,
    ref: r.referencia || '',
    fechaNotif: r.notificacion || '',
    vencCalc: r.venc_calc || r.vencimiento,
    vencFinal: r.vencimiento,
  })

  // Cargar de Supabase al montar (sincronizado entre dispositivos)
  const cargar = async () => {
    setCargando(true); setError('')
    try {
      const res = await fetch('/api/plazos', { cache:'no-store', credentials:'same-origin' })
      if (!res.ok) throw new Error('No se pudieron cargar los vencimientos')
      const data = await res.json()
      setAsuntos((data||[]).map(normaliza))
    } catch (e) { setError(e.message) }
    setCargando(false)
  }
  useEffect(() => { cargar() }, [])

  // Asunto y cliente que llegan desde un expediente del CRM
  useEffect(() => {
    if (!prefill) return
    setForm(p => ({ ...p, asunto: prefill.asunto || p.asunto, cliente: prefill.cliente || p.cliente }))
  }, [prefill])

  const f = (k,v) => setForm(p => ({ ...p, [k]:v }))

  const tipoSel = TIPOS.find(t=>t.id===form.tipo)
  const vencCalc = calcularVencimiento(tipoSel, form.fechaNotif, form.fechaLibre)
  const vencFinal = vencCalc

  const añadir = async () => {
    if (!form.asunto) { setError('Indica el asunto'); return }
    const venc = calcularVencimiento(tipoSel, form.fechaNotif, form.fechaLibre)
    if (!venc) { setError(tipoSel.libre ? 'Indica la fecha de vencimiento' : 'Indica la fecha de notificación'); return }
    ...
    try {
      const res = await fetch('/api/plazos', {
        ...
        body: JSON.stringify({
          ...
          fechaNotif: form.fechaNotif || null, vencCalc: venc, vencimiento: venc,
        })
      })
      if (!res.ok) { const j = await res.json().catch(()=>({})); throw new Error(j.error||'No se pudo guardar') }
      const nuevo = await res.json()
      setAsuntos(arr => [...arr, normaliza(nuevo)])
      setForm({ asunto:'', cliente:'', tipo:form.tipo, fechaNotif:'', fechaLibre:'' })
    } catch (e) { setError(e.message) }
    setGuardando(false)
  }
  ...
  const eliminar = async (id) => {
    setError('')
    const previo = asuntos
    setAsuntos(arr => arr.filter(a=>a.id!==id)) // optimista
    try {
      const res = await fetch('/api/plazos?id='+encodeURIComponent(id), { method:'DELETE', credentials:'same-origin' })
      if (!res.ok) throw new Error('No se pudo eliminar')
    } catch (e) { setError(e.message); setAsuntos(previo) }
  }

  const editarFecha = async (id, nuevaFecha) => {
    if (!nuevaFecha) return
    setAsuntos(arr => arr.map(a=> a.id===id ? {...a, vencFinal:nuevaFecha} : a))
    try {
      await fetch('/api/plazos', {
        method:'PATCH',
        credentials:'same-origin',
        headers:{ 'Content-Type':'application/json' },
        body: JSON.stringify({ id, vencimiento:nuevaFecha })
      })
    } catch (e) { setError('No se pudo actualizar la fecha') }
  }
  ...
  const generarPDF = async () => {
    ...
    try {
      ...
      let logoData=null, logoRatio=1
      try {
        const resp = await fetch('/logo-irm.png')
        const blob = await resp.blob()
        logoData = await new Promise((res)=>{ const fr=new FileReader(); fr.onload=()=>res(fr.result); fr.readAsDataURL(blob) })
        const dim = await new Promise((res)=>{ const im=new Image(); im.onload=()=>res({w:im.width,h:im.height}); im.onerror=()=>res(null); im.src=logoData })
        if (dim && dim.h) logoRatio = dim.w/dim.h
      } catch(e){ logoData=null }

      doc.setFillColor(...NAVY); doc.rect(0,0,PW,34,'F')
      if (logoData) {
        const lh=14, lw=lh*logoRatio
        try { doc.addImage(logoData,'PNG',M,10,lw,lh) } catch(e){ doc.setFont('times','bold'); doc.setFontSize(24); doc.setTextColor(255,255,255); doc.text('IRM', M, 20) }
      } else {
  ...
  return (
    <div style={{ display:'grid', gridTemplateColumns: isMobile?'1fr':'minmax(360px, 420px) 1fr', gap:'0', height: isMobile?'auto':'100%' }}>
      {/* FORMULARIO */}
      <div style={{ padding: isMobile?'18px 14px':'24px 26px', overflowY: isMobile?'visible':'auto', borderRight: isMobile?'none':'1px solid #e5e7eb', borderBottom: isMobile?'1px solid #e5e7eb':'none', fontFamily:'Outfit, sans-serif' }}>
        ...
        {tipoSel.libre ? (
          <div style={fieldWrap}><label style={lbl}>Fecha de vencimiento</label><input type="date" style={lineInput} value={form.fechaLibre} onChange={e=>f('fechaLibre',e.target.value)} /></div>
        ) : (
        ...
        {vencCalc && (
          <div style={{ background:'#F8F4EE', borderRadius:'8px', padding:'14px', marginBottom:'16px' }}>
            <div style={{ fontSize:'11px', color:'#9ca3af', textTransform:'uppercase', letterSpacing:'.08em', marginBottom:'4px' }}>{tipoSel.libre?'Vencimiento':'Vencimiento calculado'}</div>
        ...
        {error && <div style={{ background:'#FDECEA', border:'1px solid #F5C6C2', borderRadius:'6px', padding:'10px 12px', marginBottom:'12px', fontSize:'12px', color:'#a13226' }}>{error}</div>}

        <button onClick={añadir} disabled={guardando||!form.asunto||!vencCalc} style={{ width:'100%', padding:'14px', background:(!guardando&&form.asunto&&vencCalc)?'#0D1B2A':'#9ca3af', color:'#fff', border:'none', borderRadius:'8px', fontSize:'14px', fontWeight:600, cursor:(!guardando&&form.asunto&&vencCalc)?'pointer':'not-allowed', fontFamily:'Outfit, sans-serif' }}>{guardando?'Guardando...':'+ Añadir al calendario'}</button>
      ...
      {/* CALENDARIO */}
      <div style={{ background:'#e8eaed', overflowY: isMobile?'visible':'auto', padding: isMobile?'18px 12px':'28px' }}>
        <div style={{ maxWidth:'640px', margin:'0 auto' }}>
          ...
          {asuntos.length>0 && <p style={{ fontSize:'11px', color:'#9ca3af', marginBottom:'14px', fontFamily:'Outfit, sans-serif' }}>Los vencimientos se guardan online y se ven desde cualquier dispositivo. Elimina cada uno cuando la tarea esté hecha.</p>}
          {cargando && <p style={{ fontSize:'12px', color:'#9ca3af', fontFamily:'Outfit, sans-serif' }}>Cargando vencimientos...</p>}
                  ...
                  <div key={a.id} style={{ background:'#fff', borderRadius:'10px', overflow:'hidden', boxShadow:'0 1px 8px rgba(0,0,0,.08)', fontFamily:'Outfit, sans-serif', borderLeft:'4px solid '+c }}>
                    <div style={{ padding:'16px 20px' }}>
                      ...
                      <div style={{ display:'flex', justifyContent:'space-between', alignItems:'center', marginTop:'10px', gap:'10px' }}>
                        ...
                        <button onClick={()=>{ if(confirm('¿Eliminar este vencimiento? Hazlo cuando la tarea esté completada.')) eliminar(a.id) }} style={{ background:'none', border:'1px solid #C0392B', borderRadius:'6px', padding:'6px 12px', color:'#C0392B', fontSize:'12px', cursor:'pointer', fontFamily:'Outfit, sans-serif', fontWeight:500 }}>✓ Tarea hecha · eliminar</button>
                      </div>
                    </div>
                  </div>
                )
