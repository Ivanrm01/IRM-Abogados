...
const EMPTY = {
  ...
  seo:{metaTitle:'',metaDescription:'',keywords:'',canonical:'',ogImage:'',robots:'index, follow',schema:''}
}
...
export default function AdminPage(){
  ...
  const load=async()=>{
    setLoading(true)
    try{const r=await fetch('/api/posts');const d=await r.json();setPosts(Array.isArray(d)?d:[])}
  ...
  const onTitle=v=>{
    F('title',v)
    // Only auto-suggest slug on NEW posts and only if user hasn't typed one manually
    if(!post.id && !post._slugManual) F('slug',slugify(v))
  ...
  const onSlug=v=>{
    setPost(p=>({...p,slug:v,_slugManual:true}))
  }
  ...
  const save=async()=>{
    ...
    setSaving(true)
    const {_slugManual,...postData}=post
    const method=postData.id?'PUT':'POST'
    const url=postData.id?`/api/posts/${postData.id}`:'/api/posts'
    try{
      const r=await fetch(url,{method,headers:{'Content-Type':'application/json','x-admin-key':ADMIN_KEY},body:JSON.stringify(postData)})
      if(r.ok){flash(postData.id?'Artículo actualizado ✓':'Artículo creado ✓');await load();setView('list')}
  ...
  const edit=(p)=>{
    setPost({...p,_slugManual:true,seo:p.seo||{metaTitle:'',metaDescription:'',keywords:'',canonical:'',ogImage:'',robots:'index, follow',schema:''}})
                ...
                <div className={styles.tabPane}>
                  <div className={styles.seoBox}>
                    ...
                    <div className={styles.seoRight}>
                      ...
                      <div className={styles.seoItems}>{items.map(([l,ok])=>(
                  ...
                  <div className={styles.fg}>
                    ...
                    <input value={post.seo.metaTitle} onChange={e=>S('metaTitle',e.target.value)} placeholder={post.title} className={styles.inp}/>
                  ...
                  <div className={styles.frow}>
                    <div className={styles.fg}>
                      ...
                      <div className={styles.hint}>Editable libremente. Sin espacios, sin mayúsculas.</div>
                    </div>
                    <div className={styles.fg}>
                      <label className={styles.lbl}>Robots <span className={styles.lblHint}>(indexación)</span></label>
                      <select value={post.seo.robots} onChange={e=>S('robots',e.target.value)} className={styles.sel}>
                  ...
                  <div className={styles.fg}>
                    <label className={styles.lbl}>URL canónica <span className={styles.lblHint}>(opcional — solo si el contenido existe en otra URL)</span></label>
                    <input value={post.seo.canonical} onChange={e=>S('canonical',e.target.value)} placeholder="https://irmabogadosasesores.com/blog/..." className={styles.inp}/>
                  </div>

                  <div className={styles.fg}>
                    <label className={styles.lbl}>Schema JSON-LD personalizado <span className={styles.lblHint}>(opcional — para usuarios avanzados)</span></label>
                    <textarea value={post.seo.schema} onChange={e=>S('schema',e.target.value)} placeholder={'{\n  "@type": "FAQPage",\n  ...\n}'} rows={5} className={styles.excerptInp} style={{fontFamily:'monospace',fontSize:'12px'}}/>
                  </div>
                </div>
              )
            })()}
                ...
                <div className={styles.publishToggle}>
                  <label className={styles.toggleLbl}>
                    ...
                    <div>
                      ...
                      <div className={styles.switchDesc}>{post.published?'El artículo es visible para todos los visitantes.':'El artículo está guardado pero no lo verán los visitantes.'}</div>
                    </div>
                  </label>
                </div>
                {post.id&&(
