...
const Honorarios = dynamic(()=>import('./Honorarios'),{
  ssr:false,
  loading:()=><div style={{minHeight:'400px',display:'flex',alignItems:'center',justifyContent:'center',color:'#9ca3af',fontSize:'14px'}}>Cargando honorarios...</div>
})

const Seguridad = dynamic(()=>import('./Seguridad'),{
  ssr:false,
  loading:()=><div style={{minHeight:'400px',display:'flex',alignItems:'center',justifyContent:'center',color:'#9ca3af',fontSize:'14px'}}>Cargando seguridad...</div>
})
...
// La sesión se cierra tras 30 minutos sin actividad (el servidor hace lo mismo)
const INACTIVIDAD_MS=30*60*1000
const AVISOS_SESION={
  caducada:'Tu sesión se ha cerrado por inactividad. Vuelve a entrar.',
  cerrada:'Has cerrado la sesión.',
  todas:'Has cerrado la sesión en todos los dispositivos.',
}
const recargarCon=(motivo)=>window.location.replace(window.location.pathname+(motivo?'?sesion='+motivo:''))
...
export default function AdminPage(){
  ...
  const [faltaSeguridad,setFaltaSeguridad]=useState(false)
  const [paso,setPaso]=useState(1)                 // 1 = email y contraseña · 2 = código
  const [reto,setReto]=useState(null)
  const [codigo,setCodigo]=useState('')
  const [modoRec,setModoRec]=useState(false)       // código de recuperación en lugar del de la app
  ...
  const irAHonorarios=(datos)=>{setPrefill({destino:'honorarios',datos});setView('honorarios')}
  ...
  const load=async()=>{
    ...
    try{
      ...
      if(r.status===401){recargarCon('caducada');return}
  ...
  useEffect(()=>{(async()=>{
    const motivo=new URLSearchParams(window.location.search).get('sesion')
    if(motivo){history.replaceState(null,'',window.location.pathname);if(AVISOS_SESION[motivo])setInfo(AVISOS_SESION[motivo])}
    try{
      if(!motivo){
        const r=await fetch('/api/auth/me',{credentials:'same-origin',cache:'no-store'})
        if(r.ok){const d=await r.json();entrarAlPanel(d.usuario);setComprobando(false);return}
      }
      const st=await fetch('/api/auth/setup',{cache:'no-store'})
      const d=await st.json().catch(()=>({}))
      if(d.faltaTabla) setFaltaTabla(true)
      else{setAltaInicial(!!d.necesario);setFaltaSeguridad(!!d.faltaSeguridad)}
    }catch{ /* sin conexión: se queda en la pantalla de acceso */ }
    setComprobando(false)
  })()},[])

  // La actividad (teclear, tocar, desplazarse) mantiene viva la sesión. Si
  // pasan 30 minutos sin tocar nada, se cierra, igual que en el servidor.
  useEffect(()=>{
    if(!usuario)return
    const caducada=()=>{
      fetch('/api/auth/logout',{method:'POST',credentials:'same-origin',keepalive:true}).catch(()=>{})
      recargarCon('caducada')
    }
    const actividad=()=>{
      const ahora=Date.now()
      if(ahora-ultimaActividad.current>INACTIVIDAD_MS){caducada();return}
      ultimaActividad.current=ahora
      if(ahora-ultimoPing.current>4*60*1000){
        ultimoPing.current=ahora
        fetch('/api/auth/me',{credentials:'same-origin',cache:'no-store'})
          .then(r=>{if(r.status===401)recargarCon('caducada')}).catch(()=>{})
      }
    }
    const eventos=['pointerdown','keydown','wheel','touchstart']
    eventos.forEach(ev=>document.addEventListener(ev,actividad,{passive:true,capture:true}))
    const reloj=setInterval(()=>{if(Date.now()-ultimaActividad.current>INACTIVIDAD_MS)caducada()},60*1000)
    return()=>{eventos.forEach(ev=>document.removeEventListener(ev,actividad,{capture:true}));clearInterval(reloj)}
  },[usuario])

  const entrarAlPanel=(u)=>{
    ultimaActividad.current=ultimoPing.current=Date.now()
    setUsuario(u);setInfo('');setPaso(1);setReto(null);setCodigo('')
    setAcceso({email:'',password:'',nombre:'',password2:''})
    load()
  }
  ...
  const entrar=async()=>{
    if(entrando)return
    if(!acceso.email.trim()||!acceso.password){flash('Indica el email y la contraseña','error');return}
    setEntrando(true);setInfo('')
    try{
      ...
      if(r.ok&&d.dosPasos){
        // Contraseña correcta: falta el código de la app
        setReto(d.reto);setPaso(2);setModoRec(false);setCodigo('')
        setAcceso(a=>({...a,password:''}))
        setMsg({text:'',type:''})
      }
      else if(r.ok) entrarAlPanel(d.usuario)
      else{flash(d.error||'No se pudo entrar','error');setAcceso(a=>({...a,password:''}))}
    }catch{flash('Error de conexión','error')}
    setEntrando(false)
  }

  const verificar=async(valor)=>{
    const cod=String(valor??codigo).trim()
    if(entrando)return
    if(!cod){flash('Escribe el código','error');return}
    setEntrando(true)
    try{
      const r=await fetch('/api/auth/verificar',{
        method:'POST',credentials:'same-origin',
        headers:{'Content-Type':'application/json'},
        body:JSON.stringify({reto,codigo:cod}),
      })
      const d=await r.json().catch(()=>({}))
      if(r.ok){
        if(typeof d.codigosRestantes==='number'){
          alert('Has entrado con un código de recuperación. Te quedan '+d.codigosRestantes+
            '. Si has perdido el móvil, ve a Seguridad, desactiva la verificación en dos pasos y vuelve a activarla con el nuevo.')
        }
        entrarAlPanel(d.usuario)
      }else{
        if(d.reiniciar){setPaso(1);setReto(null)}
        setCodigo('')
        flash(d.error||'El código no es correcto','error')
  ...
  const escribirCodigo=(v)=>{
    if(modoRec){setCodigo(v.slice(0,11));return}
    const limpio=v.replace(/\D/g,'').slice(0,6)
    setCodigo(limpio)
    if(limpio.length===6)verificar(limpio)
  }
  const cambiarModoRec=()=>{setModoRec(m=>!m);setCodigo('');setMsg({text:'',type:''});setTimeout(()=>codigoRef.current?.focus(),0)}
  const volverAlPaso1=()=>{setPaso(1);setReto(null);setCodigo('');setMsg({text:'',type:''})}
  ...
  const crearPrimerAcceso=async()=>{
    ...
    try{
      ...
      const d=await r.json().catch(()=>({}))
      if(r.ok){setAltaInicial(false);entrarAlPanel(d.usuario)}
    ...
    <div className={styles.loginWrap}>
      <div className={styles.loginBox}>
        ...
        {comprobando?(
          ...
          <>
            <div className={styles.loginTitle}>Falta actualizar la base de datos</div>
            <p className={styles.loginDesc}>
              Abre Supabase → SQL Editor, ejecuta el archivo <strong>data/seguridad-schema.sql</strong> del
              proyecto y recarga esta página. Crea las tablas de sesiones, verificación en dos pasos,
              registro de accesos y registro de consentimientos de cookies.
            </p>
            <button onClick={()=>window.location.reload()} className={styles.loginBtn}>Reintentar</button>
          </>
        ):paso===2?(
          <form onSubmit={e=>{e.preventDefault();verificar()}} style={{width:'100%',display:'flex',flexDirection:'column',alignItems:'center'}}>
            <div className={styles.loginTitle}>Verificación en dos pasos</div>
            <p className={styles.loginDesc}>
              {modoRec
            ...
            <label htmlFor="codigo-2fa" className={styles.loginLabel}>{modoRec?'Código de recuperación':'Código de 6 cifras'}</label>
            <input id="codigo-2fa" ref={codigoRef} value={codigo} onChange={e=>escribirCodigo(e.target.value)}
              className={`${styles.loginInput} ${modoRec?'':styles.loginCodigo}`}
              inputMode={modoRec?'text':'numeric'} autoComplete={modoRec?'off':'one-time-code'}
              maxLength={modoRec?11:6} placeholder={modoRec?'xxxxx-xxxxx':''} autoFocus/>
            <button type="submit" disabled={entrando} className={styles.loginBtn}>
              {entrando?'Verificando...':'Verificar'}
            </button>
            <div className={styles.loginLinks}>
              <button type="button" onClick={cambiarModoRec} className={styles.loginLink}>
                {modoRec?'Usar el código de la app':'Usar un código de recuperación'}
              </button>
              <button type="button" onClick={volverAlPaso1} className={styles.loginLink}>Volver</button>
            </div>
          </form>
        ):(
          <form onSubmit={e=>{e.preventDefault();entrar()}} style={{width:'100%',display:'flex',flexDirection:'column',alignItems:'center'}}>
            <div className={styles.loginTitle}>Panel de administración</div>
            <p className={styles.loginDesc}>Acceso restringido al equipo de IRM Abogados</p>
            {info&&<div className={styles.loginInfo}>{info}</div>}
            <input type="email" placeholder="Email" value={acceso.email} onChange={e=>A('email',e.target.value)} className={styles.loginInput} autoComplete="username" autoFocus/>
            <input type="password" placeholder="Contraseña" value={acceso.password} onChange={e=>A('password',e.target.value)} className={styles.loginInput} autoComplete="current-password"/>
            <button type="submit" disabled={entrando} className={styles.loginBtn}>
        ...
        {msg.text&&<div className={styles.loginError} role="alert">{msg.text}</div>}
        {!comprobando&&!faltaTabla&&!faltaSeguridad&&!altaInicial&&(
          <p className={styles.loginPie}>Por seguridad, la sesión se cierra sola tras 30 minutos sin actividad.</p>
        )}
      </div>
    </div>
  )
    ...
    <div className={styles.admin}>
      ...
      <aside className={`${styles.sidebar} ${menuAbierto?styles.sidebarOpen:''}`} onClick={()=>setMenuAbierto(false)}>
        ...
        <nav className={styles.sideNav}>
          ...
          <button onClick={()=>{setPrefill(null);setView('honorarios')}} className={view==='honorarios'?styles.navActive:''}>
            <span>💶</span> Honorarios
          </button>
        </nav>
        ...
        <nav className={styles.sideNav}>
          <button onClick={()=>{setPrefill(null);setView('encargos')}} className={view==='encargos'?styles.navActive:''}>
        ...
        <nav className={styles.sideNav}>
          ...
          <button onClick={()=>{setPrefill(null);setView('seguridad')}} className={view==='seguridad'?styles.navActive:''}>
            <span>🛡️</span> Seguridad
            {!usuario?.dos_pasos&&<span className={styles.badgeAviso} title="Verificación en dos pasos desactivada">!</span>}
          </button>
        </nav>
      ...
      <div className={styles.main}>
        <div className={styles.topbar}>
          ...
          <div className={styles.topbarTitle}>
            {view==='usuarios'?'Accesos al panel':view==='seguridad'?'Seguridad':view==='crm'?'CRM legal':view==='honorarios'?'Honorarios':view==='list'?'Artículos del blog':view==='encargos'?'Hojas de encargo':view==='calculadoras'?'Calculadoras de garantías':view==='plazos'?'Plazos y vencimientos':post.id?'Editar artículo':'Nuevo artículo'}
          </div>
        ...
        {!usuario?.dos_pasos&&view!=='seguridad'&&(
          <div className={styles.aviso2fa}>
            <span><strong>Protege tu cuenta:</strong> activa la verificación en dos pasos. Aunque alguien averigüe tu contraseña, no podrá entrar sin el código de tu móvil.</span>
            <button onClick={()=>{setPrefill(null);setView('seguridad')}} className={styles.aviso2faBtn}>Activarla</button>
          </div>
        )}
        ...
        {view==='seguridad'&&(
          <div className={styles.toolPane}>
            <Seguridad usuario={usuario} onCambio2FA={(activo)=>setUsuario(u=>({...u,dos_pasos:activo}))} onSesionesCerradas={()=>recargarCon('todas')}/>
          </div>
        )}
        ...
        {view==='crm'&&(
          <div className={styles.toolPane}>
            <CRM onNuevoEncargo={irAEncargo} onNuevoPlazo={irAPlazo} onVerHonorarios={irAHonorarios}/>
          </div>
        )}
        ...
        {view==='honorarios'&&(
          <div className={styles.toolPane}>
            <Honorarios filtro={prefill?.destino==='honorarios'?prefill.datos:null}/>
          </div>
        )}
