...
// Tarjetas visibles por columna del pipeline antes de pulsar «Ver más»
const POR_COLUMNA = 5
...
const api = async (url, opts = {}) => {
  const res = await fetch(url, {
    cache: 'no-store',
    credentials: 'same-origin',   // la sesión viaja en la cookie
    ...opts,
    headers: { 'Content-Type': 'application/json', ...(opts.headers || {}) },
  })
  // Sesión caducada: recargamos para que el panel vuelva a la pantalla de acceso
...
export default function CRM({ onNuevoEncargo, onNuevoPlazo, onVerHonorarios }) {
  const [vista, setVista] = useState('panel')
  const [clientes, setClientes] = useState([])
  const [expedientes, setExpedientes] = useState([])
  const [actuaciones, setActuaciones] = useState([])
  const [plazos, setPlazos] = useState([])
  const [honorarios, setHonorarios] = useState([])   // conceptos del módulo Honorarios (si existe la tabla)
  ...
  const [verCol, setVerCol] = useState({})          // { fase: nº de tarjetas visibles }
  const [movidos, setMovidos] = useState([])        // expedientes movidos en esta sesión (el último, primero)
  ...
  const cargar = async () => {
    ...
    try {
      const r = await fetch('/api/plazos', { cache: 'no-store', credentials: 'same-origin' })
      if (r.ok) setPlazos(await r.json())
    } catch { /* los plazos son un extra: si fallan, el CRM sigue funcionando */ }
    try {
      const r = await fetch('/api/crm/honorarios', { cache: 'no-store', credentials: 'same-origin' })
      if (r.ok) setHonorarios(await r.json())
    } catch { /* igual que los plazos: si la tabla no existe, se usan los importes de la ficha */ }
    setCargando(false)
  }
  useEffect(() => { cargar() }, [])
  ...
  // Conceptos de honorarios por expediente (módulo Honorarios)
  const honPorExp = useMemo(() => {
    const m = {}
    for (const h of honorarios) if (h.expediente_id) (m[h.expediente_id] = m[h.expediente_id] || []).push(h)
    return m
  }, [honorarios])
  const importeHon = (h) => Number(h.importe) || (h.tipo === 'variable' && h.base && h.porcentaje ? Math.round(h.base * h.porcentaje) / 100 : 0)
  const sumaHon = (lista, f) => (lista || []).filter(f).reduce((t, h) => t + importeHon(h), 0)
  ...
  const kpis = useMemo(() => {
    ...
    return {
      ...
      // Si el expediente tiene conceptos en Honorarios, manda lo devengado y no facturado;
      // si no, la diferencia entre honorarios y facturado de la ficha (como hasta ahora)
      pendiente: expedientes.reduce((t, e) => honPorExp[e.id]
        ? t + sumaHon(honPorExp[e.id], h => h.estado === 'devengado')
        : t + (enCurso.includes(e) ? Math.max(0, (Number(e.honorarios) || 0) - (Number(e.facturado) || 0)) : 0), 0),
      facturado: expedientes.reduce((t, e) => t + (honPorExp[e.id]
        ? sumaHon(honPorExp[e.id], h => h.estado === 'facturado' || h.estado === 'cobrado')
        : (Number(e.facturado) || 0)), 0),
      conversion: resueltos ? Math.round((ganados / resueltos) * 100) : null,
    }
  }, [expedientes, clientes, abiertos, honPorExp]) // eslint-disable-line react-hooks/exhaustive-deps
  ...
  const cambiarFase = async (exp, nuevaFase) => {
    ...
    // Lo que acabas de mover se queda arriba de su nueva columna, aunque sea antiguo
    setMovidos(m => [exp.id, ...m.filter(id => id !== exp.id)])
  ...
  const borrarExpediente = async (exp) => {
    ...
    try {
      ...
      setHonorarios(l => l.filter(h => h.expediente_id !== exp.id))
  ...
  const encargoDesde = (exp) => {
    ...
    onNuevoEncargo?.({
      ...
      clienteId: exp.cliente_id || cli?.id || null,
      clienteNombre: exp.cliente_nombre || cli?.nombre || '',
      clienteNif: cli?.nif || '',
      clienteTipo: cli?.tipo || 'fisica',
      asunto: exp.titulo || '',
  ...
  // Orden de cada columna: primero lo que has movido en esta sesión; después, lo más reciente.
  // En las columnas cerradas manda la fecha de cierre; en las abiertas, la de alta en el CRM.
  const recienteKey = (e) => {
    const cerrada = e.fase === 'cerrado' || e.fase === 'perdido'
    return String((cerrada && e.fecha_cierre) || e.created_at || e.fecha_alta || '')
  }
  const ordenPipeline = (a, b) => {
    const ma = movidos.indexOf(a.id), mb = movidos.indexOf(b.id)
    if (ma !== mb) {
      if (ma === -1) return 1
      if (mb === -1) return -1
      return ma - mb
    }
    return recienteKey(b).localeCompare(recienteKey(a)) || String(b.created_at || '').localeCompare(String(a.created_at || ''))
  }

  const pipeline = () => (
    <>
      <div className={s.eyebrow}>Arrastra un expediente a otra columna para cambiar su fase · se muestran los {POR_COLUMNA} más recientes de cada fase</div>
      <div className={s.pipeline}>
        {FASES.map(f => {
          const lista = expedientes.filter(e => e.fase === f.id).sort(ordenPipeline)
          const total = lista.reduce((t, e) => t + (Number(e.honorarios) || 0), 0)
          const limite = verCol[f.id] || POR_COLUMNA
          ...
          return (
            <div key={f.id}
              ...
              <div className={s.colList}>
                ...
                {(quedan > 0 || limite > POR_COLUMNA) && (
                  <div className={s.colMore}>
                    {quedan > 0 && (
                      <button className={s.colMoreBtn}
                        onClick={() => setVerCol(v => ({ ...v, [f.id]: limite + POR_COLUMNA * 2 }))}>
                        Ver {Math.min(quedan, POR_COLUMNA * 2)} más
                        <span className={s.colMoreN}>quedan {quedan}</span>
                      </button>
                    )}
                    {limite > POR_COLUMNA && (
                      <button className={s.colLessBtn} onClick={() => setVerCol(v => ({ ...v, [f.id]: POR_COLUMNA }))}>
                        Ver menos
                      </button>
                    )}
                  </div>
      ...
      <>
        ...
        <div className={s.fichaGrid}>
          ...
          <div style={{ display: 'flex', flexDirection: 'column', gap: 16 }}>
            <div className={s.card}>
              <div className={s.cardHead}>
                <div className={s.cardTitle}>Honorarios</div>
                {onVerHonorarios && (
                  <button className={`${s.btn} ${s.btnMini}`} onClick={() => onVerHonorarios({ clienteId: e.cliente_id || null, expedienteId: e.id })}>
                    {honPorExp[e.id] ? 'Gestionar' : 'Desglosar'}
                  </button>
                )}
              </div>
              {honPorExp[e.id] ? (() => {
                const hs = honPorExp[e.id]
                const fijos = sumaHon(hs, h => h.tipo !== 'variable' && h.estado !== 'anulado')
                const vars = hs.filter(h => h.tipo === 'variable' && h.estado !== 'anulado')
                const pendFac = sumaHon(hs, h => h.estado === 'devengado')
                const sinCobrar = sumaHon(hs, h => h.estado === 'facturado')
                const cobrado = sumaHon(hs, h => h.estado === 'cobrado')
                return (
                  <div className={s.dl}>
                    <div className={s.dt}>Fijos</div><div className={s.dd}>{eur(fijos, 2)}</div>
                    {vars.map(v => (
                      <Fragment key={v.id}><div className={s.dt}>Variable</div>
                        <div className={s.dd}>
                          {v.porcentaje != null ? `${Number(v.porcentaje).toLocaleString('es-ES')} % ` : ''}
                          {v.base ? `s/ ${eur(v.base, 2)} ` : ''}→ {eur(importeHon(v), 2)}
                          <span style={{ color: '#9ca3af' }}>{v.estado === 'previsto' ? ' · en expectativa' : ''}</span>
                        </div></Fragment>
                    ))}
                    <div className={s.dt}>Pte. facturar</div>
                    <div className={s.dd} style={{ color: pendFac > 0 ? '#C0392B' : '#1A6B4A', fontWeight: 500 }}>{eur(pendFac, 2)}</div>
                    <div className={s.dt}>Sin cobrar</div><div className={s.dd}>{eur(sinCobrar, 2)}</div>
                    <div className={s.dt}>Cobrado</div><div className={s.dd}>{eur(cobrado, 2)}</div>
        ...
        <div className={s.fichaGrid}>
          ...
          <div style={{ display: 'flex', flexDirection: 'column', gap: 16 }}>
            ...
            <div className={s.card}>
              <div className={s.cardHead}><div className={s.cardTitle}>Acciones</div></div>
              <div className={s.cardBody} style={{ display: 'flex', flexDirection: 'column', gap: 8, paddingTop: 14 }}>
                {onVerHonorarios && <button className={s.btn} style={{ justifyContent: 'center' }} onClick={() => onVerHonorarios({ clienteId: c.id })}>Ver sus honorarios</button>}
