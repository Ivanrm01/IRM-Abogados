...
export default function HojaEncargo({ prefill }) {
  ...
  // Datos que llegan desde un expediente del CRM
  useEffect(() => {
    if (!prefill) return
    setD(p => ({
      ...p,
      clienteNombre: prefill.clienteNombre || p.clienteNombre,
      clienteNif: prefill.clienteNif || p.clienteNif,
      asunto: prefill.asunto || p.asunto,
      antecedentes: prefill.antecedentes || p.antecedentes,
    }))
    if (prefill.honorarios) {
      setFijos([{ concepto:'Honorarios profesionales por la dirección letrada del asunto', importe:String(prefill.honorarios) }])
    }
    if (prefill.variable) setVariable(v => ({ ...v, concepto: prefill.variable }))
  }, [prefill])
  ...
  const generarPDF = async () => {
    ...
    try {
      ...
      // Cargar el logo de IRM (public/logo-irm.png) y obtener sus dimensiones
      ...
      if (logoData) {
        // Logo a la izquierda, altura fija de 16mm, ancho proporcional
        const lh = 16, lw = lh * logoRatio
        try { doc.addImage(logoData, 'PNG', M, 13, lw, lh) } catch (e) {
          doc.setFont(FONT,'bold'); doc.setFontSize(30); doc.setTextColor(255,255,255); doc.text('IRM', M, 24)
        }
      } else {
        ...
        <div style={{ maxWidth:'600px', margin:'0 auto' }}>
          ...
          <div style={sheet}>
            ...
            <div style={{ background:'#0D1B2A', color:'#fff', padding:'22px 30px', display:'flex', justifyContent:'space-between', alignItems:'flex-start' }}>
              <div>
                <img src="/logo-irm.png" alt="IRM Abogados" style={{ height:'42px', width:'auto', display:'block' }} />
              </div>
